...
| T7 | Floor plan: Layout editor (canvas, grid, snap, zoom/pan, drag/resize/rotate, toolbar, properties, layers, minimap, undo/redo, version/publish) + operational map (dynamic from DB, states, filters, legend) | delegated | done (56e35c9; FloorPlanCanvas+editorStore+toolbar/layers/properties/minimap, LayoutEditorPage con autosave 2.5s solo borrador, useBlocker dirty-guard, publicar/restaurar con ConfirmDialog; mapa operativo dinámico desde location_occupancy; risk medium; build+lint+tsc pass) |
